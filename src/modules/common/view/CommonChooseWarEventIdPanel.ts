...
namespace Twns.Common {
    ...
    export type OpenDataForCommonChooseWarEventIdPanel = {
        currentEventIdArray     : number[];
        availableEventIdArray   : number[];
        callbackOnConfirm       : (eventIdArray: number[]) => void;
    };
    export class CommonChooseWarEventIdPanel extends TwnsUiPanel.UiPanel<OpenDataForCommonChooseWarEventIdPanel> {
        ...
        private _onTouchedBtnConfirm(): void {
            this._getOpenData().callbackOnConfirm(this._listLocation.getSelectedDataArray()?.map(v => v.warEventId).sort((v1, v2) => v1 - v2) ?? []);
            this.close();
        }
        ...
        private _updateComponentsForLanguage(): void {
            this._labelTitle.text       = Lang.getFormattedText(LangTextType.F0092, Lang.getText(LangTextType.B0462));
        ...
        private _updateListLocation(): void {
            ...
            for (const warEventId of openData.availableEventIdArray) {
                dataArray.push({ warEventId });
            }
    ...
    class PlayerRenderer extends TwnsUiListItemRenderer.UiListItemRenderer<DataForLocationRenderer> {
        ...
        protected _onDataChanged(): void {
            this._labelName.text = `${this._getData().warEventId}`;
        }
    }
}

// export default TwnsCommonChooseWarEventIdPanel;
