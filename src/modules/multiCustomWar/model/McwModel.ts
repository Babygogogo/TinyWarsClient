...
namespace TinyWars.MultiCustomWar.McwModel {
    ...
    export async function loadWar(data: ProtoTypes.WarSerialization.ISerialWar): Promise<McwWar> {
        ...
        const war = (await new McwWar().init(data)).startRunning().startRunningView() as McwWar;
        _setWar(war);
    ...
    function _setWar(war: McwWar): void {
        _war = war;
    }
    ...
    function _checkAndRequestBeginTurn(war: McwWar): void {
