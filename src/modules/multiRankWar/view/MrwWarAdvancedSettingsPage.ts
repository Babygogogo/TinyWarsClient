...
namespace TwnsMrwWarAdvancedSettingsPage {
    ...
    export class MrwWarAdvancedSettingsPage extends TwnsUiTabPage.UiTabPage<OpenDataForMrwWarAdvancedSettingsPage> {
    ...
    class InfoRenderer extends TwnsUiListItemRenderer.UiListItemRenderer<DataForInfoRenderer> {
        ...
        private async _updateComponentsForValueAsTeamIndex(playerIndex: number): Promise<void> {
            ...
            const teamIndex         = warInfo ? WarRuleHelpers.getTeamIndex(warInfo.settingsForCommon.warRule, playerIndex) : undefined;
        ...
        private async _updateComponentsForValueAsBannedCoIdArray(playerIndex: number): Promise<void> {
            const warInfo           = await this._getWarInfo();
            const currValue         = warInfo ? (WarRuleHelpers.getBannedCoIdArray(warInfo.settingsForCommon.warRule, playerIndex) || []).length : 0;
            const labelValue        = this._labelValue;
        ...
        private async _updateComponentsForValueAsInitialFund(playerIndex: number): Promise<void> {
            const warInfo           = await this._getWarInfo();
            const currValue         = warInfo ? WarRuleHelpers.getInitialFund(warInfo.settingsForCommon.warRule, playerIndex) : undefined;
            const labelValue        = this._labelValue;
        ...
        private async _updateComponentsForValueAsIncomeMultiplier(playerIndex: number): Promise<void> {
            const warInfo           = await this._getWarInfo();
            const currValue         = warInfo ? WarRuleHelpers.getIncomeMultiplier(warInfo.settingsForCommon.warRule, playerIndex): undefined;
            const labelValue        = this._labelValue;
            labelValue.text         = currValue == null ? null : `${currValue}`;
        ...
        private async _updateComponentsForValueAsEnergyAddPctOnLoadCo(playerIndex: number): Promise<void> {
            const warInfo           = await this._getWarInfo();
            const currValue         = warInfo ? WarRuleHelpers.getEnergyAddPctOnLoadCo(warInfo.settingsForCommon.warRule, playerIndex) : undefined;
            const labelValue        = this._labelValue;
            labelValue.text         = currValue == null ? null : `${currValue}`;
        ...
        private async _updateComponentsForValueAsEnergyGrowthMultiplier(playerIndex: number): Promise<void> {
            const warInfo           = await this._getWarInfo();
            const currValue         = warInfo ? WarRuleHelpers.getEnergyGrowthMultiplier(warInfo.settingsForCommon.warRule, playerIndex) : undefined;
            const labelValue        = this._labelValue;
            labelValue.text         = currValue == null ? null : `${currValue}`;
        ...
        private async _updateComponentsForValueAsMoveRangeModifier(playerIndex: number): Promise<void> {
            const warInfo           = await this._getWarInfo();
            const currValue         = warInfo ? WarRuleHelpers.getMoveRangeModifier(warInfo.settingsForCommon.warRule, playerIndex) : undefined;
            const labelValue        = this._labelValue;
            labelValue.text         = currValue == null ? null : `${currValue}`;
        ...
        private async _updateComponentsForValueAsAttackPowerModifier(playerIndex: number): Promise<void> {
            const warInfo           = await this._getWarInfo();
            const currValue         = warInfo ? WarRuleHelpers.getAttackPowerModifier(warInfo.settingsForCommon.warRule, playerIndex) : undefined;
            const labelValue        = this._labelValue;
            labelValue.text         = currValue == null ? null : `${currValue}`;
        ...
        private async _updateComponentsForValueAsVisionRangeModifier(playerIndex: number): Promise<void> {
            const warInfo           = await this._getWarInfo();
            const currValue         = warInfo ? WarRuleHelpers.getVisionRangeModifier(warInfo.settingsForCommon.warRule, playerIndex): undefined;
            const labelValue        = this._labelValue;
            labelValue.text         = currValue == null ? null : `${currValue}`;
        ...
        private async _updateComponentsForValueAsLuckLowerLimit(playerIndex: number): Promise<void> {
            const warInfo           = await this._getWarInfo();
            const currValue         = warInfo ? WarRuleHelpers.getLuckLowerLimit(warInfo.settingsForCommon.warRule, playerIndex) : undefined;
            const labelValue        = this._labelValue;
            labelValue.text         = currValue == null ? null : `${currValue}`;
        ...
        private async _updateComponentsForValueAsLuckUpperLimit(playerIndex: number): Promise<void> {
            const warInfo           = await this._getWarInfo();
            const currValue         = warInfo ? WarRuleHelpers.getLuckUpperLimit(warInfo.settingsForCommon.warRule, playerIndex) : undefined;
            const labelValue        = this._labelValue;
            labelValue.text         = currValue == null ? null : `${currValue}`;
...
export default TwnsMrwWarAdvancedSettingsPage;
