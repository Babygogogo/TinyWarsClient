...
namespace TwnsCcwMyWarListPanel {
    ...
    export class CcwMyWarListPanel extends TwnsUiPanel.UiPanel<OpenData> {
        ...
        private _onTouchedBtnNextStep(): void {
            const warId = Twns.MultiPlayerWar.MpwModel.getCcwPreviewingWarId();
            if (warId != null) {
        ...
        private async _updateGroupWarList(): Promise<void> {
            ...
            if (dataArray.every(v => v.warId != warId)) {
                Twns.MultiPlayerWar.MpwModel.setCcwPreviewingWarId(dataArray.length ? dataArray[0].warId : null);
            }
        }
        ...
        private async _createDataForListWar(): Promise<DataForWarRenderer[]> {
            ...
            for (const warId of await Twns.MultiPlayerWar.MpwModel.getMyCcwWarIdArray()) {
                dataArray.push({
                    warId,
                });
            }
        ...
        private async _createDataForCommonWarPlayerInfoPage(): Promise<OpenDataForCommonWarPlayerInfoPage> {
            return await Twns.MultiPlayerWar.MpwModel.createDataForCommonWarPlayerInfoPage(Twns.MultiPlayerWar.MpwModel.getCcwPreviewingWarId());
        }

        private async _createDataForCommonWarBasicSettingsPage(): Promise<OpenDataForCommonWarBasicSettingsPage> {
            return await Twns.MultiPlayerWar.MpwModel.createDataForCommonWarBasicSettingsPage(Twns.MultiPlayerWar.MpwModel.getCcwPreviewingWarId());
        }

        private async _createDataForCommonWarAdvancedSettingsPage(): Promise<OpenDataForCommonWarAdvancedSettingsPage> {
            return await Twns.MultiPlayerWar.MpwModel.createDataForCommonWarAdvancedSettingsPage(Twns.MultiPlayerWar.MpwModel.getCcwPreviewingWarId());
        }
    ...
    class WarRenderer extends TwnsUiListItemRenderer.UiListItemRenderer<DataForWarRenderer> {
        ...
        protected async _onDataChanged(): Promise<void> {
            ...
            if (!warSettings) {
                ...
                imgRed.visible = Twns.MultiPlayerWar.MpwModel.checkIsRedForMyWar(warSettings);
        ...
        private _onTouchTapBtnChoose(): void {
            Twns.MultiPlayerWar.MpwModel.setCcwPreviewingWarId(this._getData().warId);
        }

        private _updateState(): void {
            this.currentState = this._getData().warId === Twns.MultiPlayerWar.MpwModel.getCcwPreviewingWarId() ? Types.UiState.Down : Types.UiState.Up;
        }
    }
}
