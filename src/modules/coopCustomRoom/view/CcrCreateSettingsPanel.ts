...
namespace TinyWars.CoopCustomRoom {
    ...
    export class CcrCreateSettingsPanel extends GameUi.UiPanel<void> {
        ...
        protected _onOpened(): void {
            ...
            this._setNotifyListenerArray([
                ...
                { type: Notify.Type.CcrCreateSelfCoIdChanged,   callback: this._onNotifyCcrCreateSelfCoIdChanged },
        ...
        private _onTouchedBtnChooseCo(): void {
            CcrCreateChooseCoPanel.show({
                playerIndex : CcrModel.Create.getSelfPlayerIndex(),
                coId        : CcrModel.Create.getSelfCoId(),
            });
        }
        ...
        private _onNotifyCcrCreateSelfCoIdChanged(): void {
            this._updateBtnChooseCo();
        }
        ...
        private _updateBtnChooseCo(): void {
            const cfg               = ConfigManager.getCoBasicCfg(CcrModel.Create.getData().settingsForCommon.configVersion, CcrModel.Create.getSelfCoId());
            this._btnChooseCo.label = cfg.name;
        }

        private async _initSclPlayerIndex(): Promise<void> {
            const playersCountUnneutral = (await CcrModel.Create.getMapRawData()).playersCountUnneutral;
    ...
    class PlayerIndexRenderer extends GameUi.UiListItemRenderer<DataForPlayerIndexRenderer> {
        ...
        protected _onOpened(): void {
            this._setNotifyListenerArray([
                { type: Notify.Type.LanguageChanged,                    callback: this._onNotifyLanguageChanged },
                { type: Notify.Type.CcrCreateTeamIndexChanged,          callback: this._onNotifyCcrCreateTeamIndexChanged },
                { type: Notify.Type.CcrCreateSelfPlayerIndexChanged,    callback: this._onNotifyCcrCreateSelfPlayerIndexChanged },
            ]);
        }
        ...
        private _onNotifyCcrCreateTeamIndexChanged(): void {
            this._updateLabelName();
        }
        private _onNotifyCcrCreateSelfPlayerIndexChanged(): void {
            this._updateState();
        }
        ...
        private _updateLabelName(): void {
            ...
            if (data) {
                const playerIndex       = data.playerIndex;
                this._labelName.text    = `P${playerIndex} (${Lang.getPlayerTeamName(BwWarRuleHelper.getTeamIndex(CcrModel.Create.getWarRule(), playerIndex))})`;
            }
        }
        private _updateState(): void {
            const data          = this.data;
            this.currentState   = ((data) && (data.playerIndex === CcrModel.Create.getSelfPlayerIndex())) ? `down` : `up`;
        }
    }
    ...
    class SkinIdRenderer extends GameUi.UiListItemRenderer<DataForSkinIdRenderer> {
        ...
        protected _onOpened(): void {
            this._setNotifyListenerArray([
                { type: Notify.Type.CcrCreateSelfSkinIdChanged, callback: this._onNotifyCcrCreateSelfSkinIdChanged },
            ]);
        }
        ...
        public onItemTapEvent(): void {
            const data = this.data;
            if (data) {
                CcrModel.Create.setSelfUnitAndTileSkinId(data.skinId);
            }
        }
        private _onNotifyCcrCreateSelfSkinIdChanged(): void {
            this._updateImgColor();
        }
        ...
        private _updateImgColor(): void {
            ...
            if (data) {
                ...
                this._imgColor.source   = BwHelpers.getImageSourceForSkinId(skinId, CcrModel.Create.getSelfUnitAndTileSkinId() === skinId);
            }
        }
    }
}
