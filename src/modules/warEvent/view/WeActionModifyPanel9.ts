...
namespace TwnsWeActionModifyPanel9 {
    ...
    export class WeActionModifyPanel9 extends TwnsUiPanel.UiPanel<OpenData> {
        ...
        private _onTouchedBtnSwitchPlayerIndex(): void {
            ...
            const action        = Helpers.getExisted(openData.action.WeaSetPlayerCoEnergy);
        ...
        private _onFocusOutInputDeltaPercentage(): void {
            const action    = Helpers.getExisted(this._getOpenData().action.WeaSetPlayerCoEnergy);
        ...
        private _updateLabelPlayerIndex(): void {
            this._labelPlayerIndex.text = `P${this._getOpenData().action.WeaSetPlayerCoEnergy?.playerIndex || `??`}`;
        }

        private _updateInputDeltaPercentage(): void {
            const value                     = this._getOpenData().action.WeaSetPlayerCoEnergy?.deltaPercentage;
            this._inputDeltaPercentage.text = `${value == null ? `` : value}`;
        }

        private _updateInputMultiplierPercentage(): void {
            const value                             = this._getOpenData().action.WeaSetPlayerCoEnergy?.multiplierPercentage;
